  ...
  while (true)
	...
	if (tls_multi_process (parm.multi, &buf, &ret.to_udp_addr,
			       parm.udp_socket, &wakeup, current))
	  {
	    ...
	    check_status (stat, "write to foreground", -1);
	    if (fatal)
	      goto exit;
	  }
      ...
      if (stat > 0 && FD_ISSET (parm.sd, &reads))
	{
	  ...
	  fatal = local_sock_fatal();
	  check_status (stat, "read from foreground", -1);
	  if (stat == sizeof (tc))
	    {
...
tls_thread_send_command (int sd, int cmd)
{
  ...
  fatal = local_sock_fatal();
  check_status (stat, "write command to tls thread", -1);
  if (stat == sizeof (tc))
    return 1;
  else if (fatal)
    return -1;
...
tls_thread_rec_buf (int sd, struct tt_ret* ttr, bool do_check_status)
{
  ...
  fatal = local_sock_fatal();
  if (do_check_status)
    check_status (stat, "read buffer from tls thread", -1);
  if (stat == sizeof (*ttr))
    return 1;
  else if (fatal)
    return -1;
